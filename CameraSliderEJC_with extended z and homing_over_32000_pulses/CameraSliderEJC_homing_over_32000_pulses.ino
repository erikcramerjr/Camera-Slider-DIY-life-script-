...
//26 March 2021
//Modified with Long instead of int 
// changed variables to long for homing over 32000 pulses and use math instead of set value
//@EJC	and friend		
//25-04-2022	
...
#define maxTravDist 870
#define travDistInc 25
#define initialDur 200                      //Define duration initial setting, minimum, maximum and increment
#define minDur 10
#define maxDur 1800
#define durInc 5
...
#define objInc 50
#define minInterval 10                    //Minimum interval time between pulses in microseconds
...
volatile long encoderPos = 0;                //Current value of encoder position, contained between limits below
volatile long prevEncoderPos = 0;            //To track whether the encoder has been turned and the display needs to update
...
long travDist = maxTravDist;               //Distance to travel across slider in millimeters
...
long travelDir = 0;                          //Deifne initial travel and rotation directions
long rotDir = 0;
int rotAngle = 180;                         //Angle to rotate camera around axis

long pulsesPerMM = 40;                     //Number of motor pulses for 1mm travel
...
void homeslider ()                                          //Runs the pan mode sequence
{
  ...
  long travelPulses =   { maxTravDist * pulsesPerMM };
  //25000;               
  float interval = 380;         
   
  
  for (long i=1; i<=travelPulses; i++)                   //Pulse the motor to move the required distance in the required time
  {
...
void runPan ()                                          //Runs the pan mode sequence
{
  ...
  if (travelDir == 0)                                   //Set motor travel direction
    ...
    digitalWrite(travDirPin, HIGH);
  long travelPulses = calcTravelPulses ();               //Calculate the number of motor pulses required to move the travel distance
...
void runRotate ()                                       //Runs the rotate mode sequence
{
  ...
  long rotationPulses = calcRotationPulses ();           //Calculate the number of motor pulses required to rotate the required angle
  ...
  Serial.print("Interval: ");
  Serial.println(interval);
  for (long  i=1; i<=rotationPulses; i++)                 //Pulse the motor to rotate the required angle in the required time
  {
...
void runPanAndRotate ()                                       //Runs the pan and rotate mode sequence
{
  ...
  Serial.print("Rotation pulses: ");
  Serial.println(rotationPulses);
 long travelPerRotation = travelPulses/rotationPulses;        //Calculate how much the camera should pan for each rotation step
  ...
  for (long i=1; i<=travelPulses; i++)
  {
     if (digitalRead(leftlimit) == true && digitalRead(rightlimit) == true ){   //limit switches not reached
      ...
      digitalWrite(travStepPin, HIGH);
      long checkRotate = i % travelPerRotation;                //Check if a rotation step must be made
      if (checkRotate == 0)
        digitalWrite(rotStepPin, HIGH);
      delayMicroseconds(interval/2);
      digitalWrite(travStepPin, LOW);
...
void runTrack ()                                                //Runs the object tracking mode sequence
{
  ...
  long travelPulses = calcTravelPulses ();                       //Calculates the number of pulses required to move the travel distance
  Serial.print("Travel pulses: ");
  Serial.println(travelPulses);
  ...
  Serial.println(currentAngle);
  for (long i=1; i<=(travelPulses/2); i++)                           //Runs through movement sequence to move motors
  {
      if (digitalRead(leftlimit) == true && digitalRead(rightlimit) == true ){   //limit switches not reached
      digitalWrite(travStepPin, HIGH);
  ...
  currentAngle = 90;
  for (long i=((travelPulses/2)+1); i<=travelPulses; i++)                           //Runs through movement sequence to move motors
  {
      if (digitalRead(leftlimit) == true && digitalRead(rightlimit) == true ){   //limit switches not reached
      digitalWrite(travStepPin, HIGH);
      boolean rotatePulse = checkRot (i);
...
void inputField (int initialSetting, long lowerLimit, long upperLimit, long increment)
{
...
long calcTravelPulses ()                                       //Calculates the number of pulses required to move a certain distance
{
  long travP = travDist*pulsesPerMM;
  return travP;
}

long calcRotationPulses ()                                     //Calculate the number of pulses required to rotate a certain angle
{
  long rotP = rotAngle*pulsesPerDeg;
  return rotP;
}

boolean checkRot (long i)                                      //Used in tracking to calculate the angle required to track object
{
  boolean rotP = false;
  float deltaAngle;
...
float calcInterval (long numPulses)                                 //Calculate the interval required between pulses to achieve duration
{
  float inter = travTime*1000000/numPulses;
  return inter;
}

float calcRotInterval (long numPulses)                              //Calculate the interval required between pulses to achieve duration
{
